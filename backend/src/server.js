...
fastify.get('/api/client/config/:id', async (request, reply) => {
  ...
  try {
    const result = await client.query(`SELECT * FROM client_config WHERE org_id = $1`, [id])
...
fastify.get('/v1/organizations/:id/markdown-files', async (request, reply) => {
  ...
  try {
    const result = await client.query(
      `SELECT * FROM client_config WHERE org_id = $1`,
      [id]
    )
    if (result.rowCount === 0) {
...
fastify.post('/v1/tasks', async (request, reply) => {
  ...
  try {
    ...
    return reply.code(201).send({ success: true, task_id: result.rows[0].id, status: result.rows[0].status })
  } catch (err) {
    request.log.error(err)
    return reply.code(500).send({ success: false, error: err.message })
  } finally {
